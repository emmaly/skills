...
CASES = [
    ...
    ("--namespace= before the subcommand", 2,
     {"tool_name": "Bash", "tool_input": {"command": f'{"git"} --namespace=review commit -m "a {EM} b"'}}),
    ("--namespace with a separate value", 2,
     {"tool_name": "Bash", "tool_input": {"command": f'{"git"} --namespace review commit -m "a {EM} b"'}}),
    ("commit later in a compound command", 2,
     {"tool_name": "Bash", "tool_input": {"command": f'{"git"} status && {GC} -m "a {EM} b"'}}),
    ("compound command, clean message", 0,
     {"tool_name": "Bash", "tool_input": {"command": f'{"git"} status && {GC} -m "a. b"'}}),
    ("an absolute path to git still counts", 2,
     {"tool_name": "Bash", "tool_input": {"command": f'/usr/bin/git commit -m "a {EM} b"'}}),
