...
5. **Push and update or open the PR**: If a PR already exists, push to it. Otherwise, if more pushes are likely (iterating on CI, expecting follow-ups), open the PR as a **draft** and iterate there; with the gate clean and no more pushes expected, push and open it ready for review. CodeRabbit skips drafts unless the repo's `.coderabbit.yaml` sets `reviews.auto_review.drafts: true`; do not enable that override, and if a repo already has it, treat every draft push as costing one review until it is removed
