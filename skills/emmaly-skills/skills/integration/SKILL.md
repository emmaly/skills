...
1. **Local CR review** (MANDATORY before any push): Run `coderabbit review --base main` on the feature branch. Add `--include-untracked` when the change adds new files — untracked files are NOT reviewed by default, so a new file would otherwise reach GitHub unreviewed. Reviews can take several minutes; allow a generous timeout rather than killing the run
...
- **Scope flags**: `coderabbit review` defaults to tracked changes. `--committed` reviews only committed changes, `--uncommitted` only staged/unstaged edits, `--include-untracked` adds new files. Plain text is the default output; there is no `--plain` flag (it was removed — passing it errors out). `--agent` emits structured findings, which is the better choice when Claude is driving
