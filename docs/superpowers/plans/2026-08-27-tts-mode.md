...
> **Status: executed and superseded.** This plan was implemented on
> 2026-08-27. `skills/tts-mode/` is the source of truth; the code snippets
> below are the pre-review shape and no longer match it. Six rounds of review
> changed the lock path, the session-id validation, the state layout, the
> instruction quoting and the log capping. Read the shipped code, not these
> snippets, and treat this file as a record of how the work was sequenced.
...
Billing is per character. A fifteen-word line runs about a hundred
characters, so a turn with three lines is roughly three hundred. Against a
plan that includes 300,000 characters a month, that is on the order of a
thousand turns before the allowance is gone.

Bitrate and voice settings do not affect price. The voice is fixed to one
with a rate multiplier of 1.0, because a higher multiplier doubles the
per-character cost.
