...
> 2026-08-27. `tts-mode/` is the source of truth; the code snippets
...
- Module path mirrors repo path: `github.com/emmaly/skills/tts-mode/hooks/ttsmode`.
...
| File | Responsibility |
| --- | --- |
| `tts-mode/hooks/ttsmode/state.go` | Enable, disable, query, prune per-session state files |
| `tts-mode/hooks/ttsmode/state_test.go` | Tests for the above |
| `tts-mode/hooks/ttsmode/hook.go` | Read hook payload, decide, emit instruction or nothing |
| `tts-mode/hooks/ttsmode/hook_test.go` | Tests for the above |
| `tts-mode/hooks/ttsmode/secret.go` | Resolve the API key from environment or env file |
| `tts-mode/hooks/ttsmode/secret_test.go` | Tests for the above |
| `tts-mode/hooks/ttsmode/speak.go` | Synth and Player interfaces, ElevenLabs client, command wiring |
| `tts-mode/hooks/ttsmode/speak_test.go` | Tests using fakes |
| `tts-mode/hooks/ttsmode/main.go` | Subcommand dispatch only |
| `tts-mode/hooks/ttsmode/main_test.go` | Dispatch tests |
| `tts-mode/hooks/run-ttsmode.sh` | Build-if-stale, run the hook subcommand |
| `tts-mode/hooks/tts-say.sh` | Build-if-stale, background and serialize playback |
| `tts-mode/hooks/hooks.json` | Registers UserPromptSubmit and SessionStart |
| `tts-mode/commands/tts.md` | The `/tts` slash command |
| `tts-mode/.claude-plugin/plugin.json` | Plugin manifest |
| `tts-mode/README.md` | What it is, how to turn it on, what it costs |
...
**Files:**
- Create: `tts-mode/hooks/ttsmode/go.mod`
- Create: `tts-mode/hooks/ttsmode/state.go`
- Test: `tts-mode/hooks/ttsmode/state_test.go`
...
```bash
mkdir -p tts-mode/hooks/ttsmode
cd tts-mode/hooks/ttsmode
cat > go.mod <<'EOF'
module github.com/emmaly/skills/tts-mode/hooks/ttsmode
...
Run: `cd tts-mode/hooks/ttsmode && go test ./...`
...
```bash
git add tts-mode/hooks/ttsmode/go.mod tts-mode/hooks/ttsmode/state.go tts-mode/hooks/ttsmode/state_test.go
...
**Files:**
- Create: `tts-mode/hooks/ttsmode/hook.go`
- Test: `tts-mode/hooks/ttsmode/hook_test.go`
...
```bash
git add tts-mode/hooks/ttsmode/hook.go tts-mode/hooks/ttsmode/hook_test.go
...
**Files:**
- Create: `tts-mode/hooks/ttsmode/secret.go`
- Test: `tts-mode/hooks/ttsmode/secret_test.go`
...
```bash
git add tts-mode/hooks/ttsmode/secret.go tts-mode/hooks/ttsmode/secret_test.go
...
**Files:**
- Create: `tts-mode/hooks/ttsmode/speak.go`
- Test: `tts-mode/hooks/ttsmode/speak_test.go`
...
```bash
git add tts-mode/hooks/ttsmode/speak.go tts-mode/hooks/ttsmode/speak_test.go
...
**Files:**
- Create: `tts-mode/hooks/ttsmode/main.go`
- Test: `tts-mode/hooks/ttsmode/main_test.go`
...
```bash
git add tts-mode/hooks/ttsmode/main.go tts-mode/hooks/ttsmode/main_test.go
...
**Files:**
- Create: `tts-mode/hooks/run-ttsmode.sh`
- Create: `tts-mode/hooks/tts-say.sh`
- Create: `tts-mode/hooks/hooks.json`
- Create: `tts-mode/commands/tts.md`
- Create: `tts-mode/.claude-plugin/plugin.json`
- Create: `tts-mode/README.md`
...
Create `tts-mode/hooks/run-ttsmode.sh`, modeled on `run-plaincheck.sh`:
...
```json
{
  ...
  "homepage": "https://github.com/emmaly/skills",
  "repository": "https://github.com/emmaly/skills",
  "license": "MIT",
...
```bash
chmod +x tts-mode/hooks/run-ttsmode.sh tts-mode/hooks/tts-say.sh

# The off case must print nothing at all.
CLAUDE_CODE_SESSION_ID=smoke bash tts-mode/hooks/run-ttsmode.sh hook <<< '{"session_id":"smoke"}' | wc -c
# Expected: 0

CLAUDE_CODE_SESSION_ID=smoke bash tts-mode/hooks/run-ttsmode.sh on
CLAUDE_CODE_SESSION_ID=smoke bash tts-mode/hooks/run-ttsmode.sh hook <<< '{"session_id":"smoke"}' | head -5
# Expected: the instruction block, naming tts-say.sh

bash tts-mode/hooks/tts-say.sh "Smoke test line, this should be audible."
# Expected: returns immediately, speaks a moment later

CLAUDE_CODE_SESSION_ID=smoke bash tts-mode/hooks/run-ttsmode.sh off
```

- [ ] **Step 9: Commit**

```bash
git add tts-mode .claude-plugin/marketplace.json
